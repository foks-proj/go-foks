...
// Failed and doesn't block the rest of its channel; an explicit retry (the manual
...
func TestRTOutboxFailedRowStepsAside(t *testing.T) {
	...
	// The rejection is recorded as its status code, not its message. Error
	// strings carry names (a channel name, or whatever a caller put in a
	// generic RT error) and the outbox row is stored unencrypted, so the
	// message must not be persisted -- only a code, which cannot carry one.
	require.Equal(t, "BAD_ARGS_ERROR", rows[0].LastError)
	require.NotContains(t, rows[0].LastError, "simulated semantic rejection")
	require.Equal(t, lcl.RTOutboxState_Queued, rows[1].State)
...
func TestRTDegradedReads(t *testing.T) {
	...
	// result that would read as a genuinely empty thread.
