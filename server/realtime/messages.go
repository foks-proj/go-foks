...
func (s *messageSender) channelID() int64 { return int64(s.arg.ChannelID.Short()) }
...
func (s *messageSender) checkEncryptionRole(box proto.RTBoxRG) error {
	msgRole, err := core.ImportRole(box.Rg.Role)
	if err != nil {
		return err
	}
...
func (s *messageSender) insertMessage(
	...
	box := s.arg.Body.Encrypted()
	err = s.checkEncryptionRole(box)
	if err != nil {
		return zed, err
	}
	boxBytes, err := core.EncodeToBytes(&box.Box)
	if err != nil {
		return zed, err
	}
	roleType, vizLevel, err := box.Rg.Role.ExportToDB()
	if err != nil {
		return zed, err
	}
	typ, err := s.arg.Typ.ExportToDB()
	if err != nil {
		return zed, err
	}
	...
	err = s.tx.QueryRow(
		...
		`INSERT INTO messages_enc
			(short_host_id, channel_id, seq, typ, msg_box, ptk_gen,
...
func (s *messageSender) run(m shared.MetaContext) (*rem.RTSendRes, error) {
	...
	if s.arg.ExpectedPrevSeq != nil && int64(*s.arg.ExpectedPrevSeq) != s.prevSeq {
...
func readThread(
	...
	case proto.RTThreadDir_Backward:
		sql = `SELECT me.seq, me.typ, me.msg_box, me.ptk_gen,
	...
	for rows.Next() {
		...
		err = core.DecodeFromBytes(&box.Box, msgBoxRaw)
		if err != nil {
			return nil, err
		}
		...
		msg := rem.RTMessage{
			...
			Seq:           proto.RTMsgSeq(seq),
			Typ:           typ,
			Body:          proto.NewRTMsgBodyWithEncrypted(box),
			SenderPartyID: &pid,
			SentAtTime:    proto.ExportTime(sentAtTime),
