...
// row (semantic rejection) steps aside so it doesn't block the rest of the
...
// changed.
// errToOutboxCode reduces an attempt's rejection to the status code that gets
// persisted for it. Only the code is stored: error *messages* carry names
// (RTAmbiguousChannelError prints a channel name, and the generic RT errors
// are free text), and the outbox row lives in a local DB that stores values
// unencrypted -- the same reason the queued message beside it stays sealed.
// The full message still reaches the logs where the attempt failed.
func errToOutboxCode(e error) proto.StatusCode {
	if e == nil {
		return proto.StatusCode_OK
	}
	// An error ErrorToStatus does not recognise lands in the default arm,
	// whose code is OK(0) -- which here would read as "no rejection". Map it
	// to GENERIC_ERROR so a recorded rejection stays distinguishable from
	// the absence of one.
...
func (d *Minder) setOutboxState(
	...
	entry.State = state
	entry.Attempts++
	entry.LastErrCode = errToOutboxCode(attemptErr)
	if !stateChanged {
...
// ack cannot corrupt the mapping that later prev-pointer validations consult.
...
// steps past it, so one undeliverable message doesn't block the rest of the
// channel.
//
...
func (d *Minder) ListOutbox(m MetaContext) ([]lcl.RTOutboxRowView, error) {
	...
	for _, e := range entries {
		ret = append(ret, lcl.RTOutboxRowView{
			...
			State:     e.State,
			Attempts:  e.Attempts,
			LastError: outboxCodeString(e.LastErrCode),
